...
# Optimized settings for GitHub Actions (7GB RAM)
MAX_PDFS_PER_RUN = int(os.environ.get("MAX_PDFS_PER_RUN", "50"))  # অনেক বেশি PDF প্রসেস করতে পারবে
PDF_BATCH_SIZE = int(os.environ.get("PDF_BATCH_SIZE", "100"))     # বড় ব্যাচ সাইজ
MAX_FILES_PER_COMMIT = int(os.environ.get("MAX_FILES_PER_COMMIT", "100"))
PDF_SLEEP_BETWEEN = int(os.environ.get("PDF_SLEEP_BETWEEN", "1")) # কম স্লিপ টাইম
MAX_RETRIES = int(os.environ.get("MAX_RETRIES", "3"))

# Parallel processing settings
MAX_PARALLEL_PDFS = int(os.environ.get("MAX_PARALLEL_PDFS", "3"))  # একসাথে ৩টি PDF প্রসেস
MAX_WORKERS = int(os.environ.get("MAX_WORKERS", "4"))              # থ্রেড পুল সাইজ

# High-quality image settings (RAM allows it)
IMAGE_ZOOM = float(os.environ.get("IMAGE_ZOOM", "4.0"))  # হাই কোয়ালিটি ইমেজ
IMAGE_DPI = int(os.environ.get("IMAGE_DPI", "300"))      # হাই DPI
...
class HFHelper:
    ...
    def upload_batch(self, local_folder: str, hf_path: str, batch_name: str) -> bool:
        """Upload with larger batch size for GitHub Actions"""
        try:
            ...
            print(f"[HF] Uploading {len(png_files)} files (Max: {MAX_FILES_PER_COMMIT})")
...
class PDFProcessor:
    ...
    def process_pdf_optimized(self, pdf_url: str, pdf_path: str, pdf_number: int, 
                              ...
                              mongo_helper: MongoDBHelper) -> Dict:
        """Process PDF with GitHub Actions optimized settings"""
        ...
        try:
            ...
            print(f"[PDF {pdf_number}] {total_pages} pages total")
            ...
            for batch_start in range(0, total_pages, PDF_BATCH_SIZE):
                ...
                try:
                    ...
                    for page_num in range(batch_start, batch_end):
                        ...
                        # High quality rendering (GitHub Actions can handle it)
                    ...
                    # Upload batch
                    if pages_in_batch > 0:
                        upload_success = self.hf.upload_batch(
                            str(batch_folder),
                            hf_path,
...
class TafsirProcessor:
    ...
    def process_archive_optimized(self, archive_item: Dict) -> bool:
        ...
        try:
            ...
            print(f"[Archive] Found {len(pdf_urls)} PDFs")
            
            # Download PDFs in parallel
            print(f"[Archive] Downloading {len(pdf_urls)} PDFs in parallel...")
            download_results = PDFDownloader.download_parallel(pdf_urls)
    ...
    def run(self):
        ...
        print(f"[Main] Max PDFs per run: {MAX_PDFS_PER_RUN}")
        print(f"[Main] PDF Batch Size: {PDF_BATCH_SIZE}")
        print(f"[Main] Image Quality: {IMAGE_ZOOM}x zoom @ {IMAGE_DPI} DPI")

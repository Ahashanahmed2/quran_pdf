...
# 🔥 FIX 1: safe_rmtree ফাংশন - retry mechanism for temp folder cleanup
def safe_rmtree(path, retries=5):
    """Retry mechanism for removing temp directories"""
    ...
    for i in range(retries):
        try:
            ...
            print(f"[WARN] rmtree attempt {i+1} failed for {path}: {e}", flush=True)
        
        # Force garbage collection
        gc.collect()
        time.sleep(1 + i)
    
    # Last attempt with ignore_errors
    try:
        shutil.rmtree(path, ignore_errors=True)
    except:
        pass
...
# 🔥 FIX 2: cleanup_old_temp_files - remove old temp files on startup
def cleanup_old_temp_files(max_age_minutes=60):
    """Clean up old temp files and folders from TEMP_DIR"""
    try:
        now = time.time()
        cleaned_count = 0
                ...
                if item.is_file() or item.is_dir():
                    ...
                    if age > max_age_minutes * 60:
                        if item.is_dir():
                            ...
                            os.remove(item)
                        cleaned_count += 1
                        print(f"[CLEANUP] Removed old item: {item.name}", flush=True)
            except Exception as e:
                print(f"[CLEANUP WARN] {item}: {e}", flush=True)
        
        if cleaned_count > 0:
            print(f"[CLEANUP] Cleaned {cleaned_count} old items", flush=True)
    except Exception as e:
        print(f"[CLEANUP ERROR] {e}", flush=True)

# 🔥 FIX 3: auto_cleanup_if_needed - auto cleanup when disk space is low
def auto_cleanup_if_needed():
    """Auto cleanup if disk space is low"""
    free_mb = check_disk_space()
    if free_mb < 200:  # Less than 200MB free
        print(f"[AUTO CLEANUP] Low disk space ({free_mb}MB), cleaning temp...", flush=True)
        cleanup_old_temp_files(10)  # Clean files older than 10 minutes
        return True
    return False
...
# 🔥 FIX 8: Improved download_pdf_stream with proper cleanup
def download_pdf_stream(url):
    print(f"[DEBUG] Downloading: {url[:80]}...", flush=True)
    
    # 🔥 FIX: Use with statement for auto cleanup
    ...
    try:
        ...
        for batch_start in range(start_page, total_pages, PDF_BATCH_SIZE):
            ...
            try:
                ...
                # 🔥 FIX: Improved cleanup with safe_rmtree
        ...
        if pdf_path:
            try:
                ...
                print(f"[WARN] Could not remove PDF {pdf_path}: {e}", flush=True)
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body { font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background: linear-gradient(135deg, #1a5f7a 0%, #0d3b4c 100%); min-height: 100vh; padding: 20px; }
        .container { max-width: 950px; margin: 0 auto; background: white; border-radius: 20px; padding: 30px; box-shadow: 0 20px 60px rgba(0,0,0,0.3); }
        h1 { color: #1a5f7a; text-align: center; margin-bottom: 10px; font-size: 2em; }
        .subtitle { text-align: center; color: #666; margin-bottom: 30px; }
        .form-group { margin-bottom: 20px; }
        label { display: block; margin-bottom: 8px; font-weight: bold; color: #333; }
        textarea { width: 100%; padding: 12px 15px; border: 2px solid #ddd; border-radius: 10px; font-size: 14px; min-height: 180px; font-family: monospace; }
        textarea:focus { outline: none; border-color: #1a5f7a; }
        .btn { background: linear-gradient(135deg, #1a5f7a 0%, #0d3b4c 100%); color: white; border: none; padding: 15px 30px; border-radius: 10px; font-size: 16px; font-weight: bold; cursor: pointer; width: 100%; transition: transform 0.2s; }
        ...
        .info-box { background: #e7f3ff; border: 1px solid #b8daff; border-radius: 10px; padding: 15px; margin-bottom: 20px; }
        .quick-buttons { display: flex; gap: 10px; margin-bottom: 15px; flex-wrap: wrap; }
        .quick-btn { background: #e9ecef; border: 1px solid #ced4da; padding: 8px 15px; border-radius: 5px; cursor: pointer; font-size: 12px; }
        ...
        .progress-bar-container { background: #e9ecef; height: 25px; border-radius: 15px; margin: 8px 0; overflow: hidden; }
        .progress-bar { background: linear-gradient(135deg, #1a5f7a, #0d3b4c); height: 25px; border-radius: 15px; text-align: center; color: white; font-size: 13px; line-height: 25px; width: 0%; }
        .progress-bar-page { background: linear-gradient(135deg, #28a745, #20c997); height: 20px; border-radius: 10px; text-align: center; color: white; font-size: 11px; line-height: 20px; width: 0%; }
        .task-card { background: #f8f9fa; padding: 15px; margin-bottom: 15px; border-radius: 10px; border-left: 4px solid #1a5f7a; }
        .page-progress { margin-top: 10px; padding: 10px; background: white; border-radius: 8px; border: 1px solid #dee2e6; }
        .memory-info { background: #1a1a2e; color: white; padding: 8px 12px; border-radius: 8px; margin-top: 10px; font-size: 13px; }
        .memory-bar-container { background: rgba(255,255,255,0.2); height: 8px; border-radius: 4px; margin-top: 5px; overflow: hidden; }
        .memory-bar { background: #00d2ff; height: 8px; border-radius: 4px; width: 0%; transition: width 0.3s; }
        .memory-warning { background: #ffc107; color: #000; padding: 2px 8px; border-radius: 4px; font-size: 11px; margin-left: 8px; }
        .memory-critical { background: #dc3545; color: white; padding: 2px 8px; border-radius: 4px; font-size: 11px; margin-left: 8px; }
    </style>
</head>
<body>
    <div class="container">
...
async def lifespan(app: FastAPI):
    ...
    print("[STARTUP] Cleaning old temp files...", flush=True)
    cleanup_old_temp_files(60)  # Remove files older than 60 minutes
    ...
    # 🔥 SHUTDOWN CLEANUP
    print("[SHUTDOWN] Final cleanup...", flush=True)
    cleanup_old_temp_files(0)  # Remove all temp files on shutdown
...
async def check_folder(folder_name: str):
    clean_name = folder_name.replace(' ', '').replace('+', '').replace('(', '').replace(')', '')
